...
void			crosses(t_vec2 vec2, t_mlx *mlx, int colour)
{
	...
	n = 0;
	while ((p0.x * p0.x * p0.x * p0.x) +
	(p0.y * p0.y * p0.y * p0.y) <= 4 && n < mlx->fract->maxiter)
	{
		xtp = (p0.x * p0.x * p0.x * p0.x) - (p0.y * p0.y * p0.y * p0.y) + v0.x;
		p0.y = (4 * p0.x * p0.y) + v0.y;
		p0.x = xtp;
		n++;
	}
	...
	while (n >= mlx->fract->maxiter)
		n -= mlx->fract->maxiter;
	plot(vec2.x, vec2.y,
	doubletocolour(n / (float)mlx->fract->maxiter, colour), mlx);
}
